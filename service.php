...
class ProductService
{
    ...
    private static function extractFromJavaScriptData(string $html): ?array
    {
        // Try to extract from data-page attribute (Vue.js/Inertia pattern)
        if (preg_match('/data-page="([^"]+)"/', $html, $matches)) {
            ...
            if ($data && isset($data['props']['product']['data'])) {
                $productData = $data['props']['product']['data'];
        ...
        // Try to extract from embedded JSON in script tags
        if (preg_match_all('/<script[^>]*>(.*?)<\/script>/s', $html, $matches)) {
            foreach ($matches[1] as $scriptContent) {
                // Look for product object patterns
                if (preg_match('/"product"\s*:\s*\{[^}]*"name"\s*:\s*"([^"]+)"[^}]*"price"\s*:\s*([0-9.]+)/s', $scriptContent, $productMatch)) {
                    return [
                        'title' => $productMatch[1] ?? null,
                        'price' => $productMatch[2] ?? null,
                        'image' => null,
                    ];
                }
            }
    ...
    private static function extractImage(string $html): ?string
    {
        $patterns = [
            ...
            // Amazon-specific patterns (keep for backward compatibility)
            '/<img[^>]*class="[^"]*a-dynamic-image[^"]*"[^>]*data-src="([^"]+)"/i',
            '/<img[^>]*id="landingImage"[^>]*data-src="([^"]+)"/i',
            '/<img[^>]*id="imgBlkFront"[^>]*data-src="([^"]+)"/i',
            '/<img[^>]*id="main-image"[^>]*src="([^"]+)"/i',
            '/<img[^>]*class="[^"]*itemNo0[^"]*"[^>]*src="([^"]+)"/i',
            '/<img[^>]*data-old-hires="([^"]+)"/i',
            '/<img[^>]*srcset="([^"]+)"[^>]*class="[^"]*a-dynamic-image[^"]*"/i',
            '/"large":"([^"]+\.jpg[^"]*)"/i',
            ...
            // Alt text patterns
            '/<img[^>]*alt="[^"]*product[^"]*image[^"]*"[^>]*src="([^"]+)"/i',
            '/<img[^>]*alt="[^"]*main[^"]*image[^"]*"[^>]*src="([^"]+)"/i',
        ];
        ...
        $contentPatterns = [
            '/<div[^>]*class="[^"]*product[^"]*"[^>]*>.*?<img[^>]*src="([^"]+)"/s',
            '/<div[^>]*class="[^"]*main[^"]*"[^>]*>.*?<img[^>]*src="([^"]+)"/s',
            '/<div[^>]*id="[^"]*product[^"]*"[^>]*>.*?<img[^>]*src="([^"]+)"/s',
        ];
